...
class GameTest < Minitest::Test
  ...
  def test_score_without_bonus
    assert_equal 71, Game.new('6,3,9,0,0,3,8,1,6,3,1,2,5,1,8,0,1,6,4,4').point_result
  end

  def test_two_shots_in_the_last_frame
    assert_equal 120, Game.new('6,3,9,0,0,3,8,2,7,3,X,9,1,8,0,X,1,2').point_result
  end

  def test_only_one_shot_after_strike
    assert_equal 142, Game.new('6,3,9,0,0,3,8,2,7,3,X,9,1,8,0,X,0,X,8').point_result
  end

  def test_no_shot_after_strike
    assert_equal 144, Game.new('6,3,9,0,0,3,8,2,7,3,X,9,1,8,0,X,X,0,X').point_result
  end

  def test_no_shot_after_spare
    assert_equal 153, Game.new('6,3,9,0,0,3,8,2,7,3,X,9,1,8,0,X,X,9,1').point_result
  end
